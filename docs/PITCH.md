...
> **配套**：指标见 [eval/reports/BASELINE.md](../eval/reports/BASELINE.md) · 场景题见 [qa/qa-m6.md](./qa/qa-m6.md) §M6.4 · 业务背景见 [scenario/SCENARIO.md](./scenario/SCENARIO.md)
...
> 这个项目我不是堆概念，而是按 **检索质量 → 生成安全 → Agent 分工 → 会话状态** 一层层加能力，每一层都有 eval 或 trace 能证明。如果让我继续做，我会用 **soft-fallback 压误拒答**（M6.27 已做，数字待刷）、补 **知识库工程（版本/增量）**，以及 README 简历化。
...
- [x] 30 秒内说清 **为谁、解决什么痛点**
- [x] 能报 **Recall@3、幻觉率 17.2%→11.5%** 两个数字
- [x] 能画 **safe_check → planning → coordinator → specialists → merge**
- [x] 能区分 **三 tool、safe_response vs abstain、三种记忆**
- [x] 被追问「你的贡献」：从 M4 eval/CRAG 到 M6 Agent/记忆，按里程碑讲

**下一步**：README 简历化（U-019）已完成；可继续巩固本 PITCH，或按需推进 [interview/upgrades/backlog.md](./interview/upgrades/backlog.md) 中的生成评测刷新与 Agent 端到端评测。
